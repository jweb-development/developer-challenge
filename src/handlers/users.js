...
const defaultFileEncoder = bodyParser.urlencoded({
  extended: true,
  limit: '500kb'
})
...
app.get('/v1/user', defaultFileParser, defaultFileEncoder, async (req, res) => {
  try {
    ...
    if (!results) {
      return responseHandler.handleNotFound('users', req, res)
    }
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const results = await users.getUser(id)

      if (!results) {
        return responseHandler.handleNotFound('user', req, res)
      }

      const response = {
        data: results,
        message: `Supervisor ${results[0].firstName}  ${results[0].lastName} found.`
      }
...
app.delete(
  ...
  async (req, res) => {
    try {
      ...
      return responseHandler.handleSuccess(results, req, res)
    } catch (err) {
      console.error(err.message)
      const response = { data: null, message: 'Internal Server Error' }
      return responseHandler.handleError(response, req, res)
